...
#    return [[math.exp(float(-j)/1440) for j in i] for i in x]
    return [(((float(maxvalue_of_time)-11*np.array(i))*float(nrg))/org + nrm) for i in x]
#
...
i13 = LSTM(32, return_sequences = True)(i12)
i14 = LSTM(32)(i12)
...
#i12 = dot([i12, i12], axes = -1)
multimodal = concatenate([i14, i21, i31], axis = -1)
...
print(model.summary())
history = model.fit([X1_train,X2_train, X3_train], y_train, validation_data=([X1_v,X2_v, X3_v], y_v), epochs= 20, batch_size=32, verbose = 1)
...
xaxis = []
yaxis = []
for i in range(0, maxvalue_of_time, 100):
#    print(i)
    x22 = newrange([[i]])
